...
class ScriptArguments:
    ...
    optimizer_type: Optional[str] = field(default='adama', metadata={"help": "which optimizer to use,use 'adama' or 'adafactor'"})
    ...
    save_freq: Optional[int] = field(default=2, metadata={"help": "n steps to save the model"})
    output_dir: Optional[str] = field(default="/root/autodl-tmp/output_model/test01", metadata={"help": "n steps to save the model"})
...
'adama' or 'adafactor'
optimizer = None
if script_args.optimizer_type == 'adama':
...
elif script_args.optimizer_type == 'adafactor':
    print("load Adafactor optimizer")
    optimizer = Adafactor(
...
# Create the pipeline
model_se = LlamaForCausalLM.from_pretrained(script_args.reward_model_name,
                                            load_in_8bit=True,
                                            device_map={"": current_device},
                                            # quantization_config=nf4_config,
)
tokenizer_se = LlamaTokenizer.from_pretrained(script_args.tokenizer_name)
se_generator = pipeline("text-generation",
                        model=model_se, #script_args.reward_model_name,
                        device_map={"": current_device},
                        model_kwargs={"load_in_8bit": True},
...
for epoch in range(script_args.epoch_num):
    for batch_num, batch in tqdm(enumerate(ppo_trainer.dataloader)):
        if batch_num >= script_args.steps:
            break
        ...
        se_rewards = [torch.tensor(se_reward_number_t5(se_text[0]['generated_text'])) for se_text in pipeline_output]
        print(se_rewards)
