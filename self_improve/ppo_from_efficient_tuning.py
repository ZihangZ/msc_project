...
class PPOTrainerForChatGLM(PPOTrainer, PeftTrainer):
    ...
    # 7_13_2023
    def se_reward(self, se_output_ids):
        
        # try:
        print(se_output_ids[0])
        print(se_output_ids[0].shape)
        se_reward = torch.tensor(0,requires_grad=False).cuda()
        se_output = self.tokenizer.decode(se_output_ids[0], skip_special_tokens=False)
        print("self evaluation output: " + se_output)
        if se_output == "Yes":
            se_reward = torch.tensor(1,requires_grad=False).cuda()
        elif se_output == "No":
            se_reward = torch.tensor(-1,requires_grad=False).cuda()
        se_reward = se_reward.to(torch.float32)
        # except Exception as e:
        #     print(e)
        #     print("se_reward generating failed")
            
        return se_reward
    
    # 7_13_2023
    def generate_se_input(self, queries: List[torch.LongTensor], responses: List[torch.LongTensor]):
        
        prompt_template = """Is the answer to the question correct? If you think the answer is correct, reply YES. If you think the answer is wrong, reply NO. Otherwise, reply NONE. The question is: {question}. The answer is:{response}"""
        
        # se_prompt = prompt_template.replace("{question}", query).replace("{response}", responses)
        
        # se_input = {}
        se_prompts = []
        try:
            for i in range(len(queries)):
                query = self.tokenizer.decode(queries[i], skip_special_tokens=False)
                response = self.tokenizer.decode(responses[i], skip_special_tokens=False)
                se_prompt = prompt_template.replace("{question}", query).replace("{response}", response)
                print(se_prompt)
                se_prompt_ids = self.tokenizer.encode(text=se_prompt, add_special_tokens=False)
                se_prompts.append(se_prompt_ids)
        except Exception as e:
            print(e)
            print("se_prompt generating failed")
            
        # print(se_prompts)
        # se_input["input_ids"] = torch.tensor(se_prompts, device=self.current_device)
        # se_input["attention_mask"] = self.data_collator.get_attention_masks(se_prompts, device=self.current_device)
        # se_input["position_ids"] = self.data_collator.get_position_ids(se_prompts, device=self.current_device)
        # print(se_input)
        return se_prompts
    ...
    def ppo_train(self, max_target_length: int) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_world_process_zero()):
            ...
            for _ in range(self.config.gradient_accumulation_steps):
                ...
                # 7_13_2023
                # Compute rewards by self evaluation
                se_prompt_input = self.generate_se_input(queries, responses)
                se_rewards = []
                for se_i in se_prompt_input:
                    se_i_len = len(se_i)
                    input_ids_i = torch.tensor(se_i,device=self.current_device).view(1,-1)
                    # print(input_ids_i)
                    attention_mask_i = self.data_collator.get_attention_masks(input_ids_i, device=self.current_device)
                    position_ids_i = self.data_collator.get_position_ids(input_ids_i, device=self.current_device)
                    se_length_sampler = LengthSampler(se_i_len, se_i_len+1)
                    se_output_i = self.generate({"input_ids": input_ids_i}, length_sampler=se_length_sampler, return_prompt=False, **gen_kwargs)
                    se_reward_i = self.se_reward(se_output_i)
                    se_rewards.append(se_reward_i)
                print(se_rewards)
    ...
    # 7_13_2023
    def generate_for_se(self, query_tensor: torch.Tensor, **generation_kwargs):
        """
        ...
        response = unwrapped_model.generate(**query_tensor, **generation_kwargs)
